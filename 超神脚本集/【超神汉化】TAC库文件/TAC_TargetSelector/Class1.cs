...
//                Game.PrintChat("Best target is: +bestTarget);
        ...
        {{
            _Config = Config;
            Config.AddSubMenu(new Menu("目标选择","tac_SimpleTS"));
            Config.SubMenu("tac_SimpleTS").AddSubMenu(new Menu("目标","targets"));

            foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy))
            {{
                Config.SubMenu("tac_SimpleTS").SubMenu("targets").AddItem(new MenuItem("priority" + enemy.ChampionName, enemy.ChampionName).SetShared().SetValue(new Slider(FindPrioForTarget(enemy.ChampionName), 5, 1)));
            }
            Config.SubMenu("tac_SimpleTS").SubMenu("targets").AddItem(new MenuItem("autoArrange","自动排列").SetValue(false));
            Config.SubMenu("tac_SimpleTS").SubMenu("targets").Item("autoArrange").ValueChanged += prioritizeChampion;


            Config.SubMenu("tac_SimpleTS").AddItem(new MenuItem("currentMode", 模式").SetValue(new StringList(Enum.GetNames(typeof(TargetingMode)), 2)));
            Config.SubMenu("tac_SimpleTS").Item("currentMode").ValueChanged += currentMode;

            Config.SubMenu("tac_SimpleTS").AddItem(new MenuItem("FocusSelected", 锁定目标").SetShared().SetValue(true));
            Config.SubMenu("tac_SimpleTS").AddItem(
