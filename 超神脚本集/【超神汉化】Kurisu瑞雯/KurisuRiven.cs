...
namespace KurisuRiven
{
    internal class KurisuRiven
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            // Target Selector
            var menuTS = new Menu("目标选择", "tselect");
            TargetSelector.AddToMenu(menuTS);
            config.AddSubMenu(menuTS);

            // Orbwalker
            var menuOrb = new Menu("走砍", "orbwalker");
            orbwalker = new Orbwalking.Orbwalker(menuOrb);
            config.AddSubMenu(menuOrb);

            var menuK = new Menu("热键", "demkeys");
            menuK.AddItem(new MenuItem("combokey", "连招")).SetValue(new KeyBind(32, KeyBindType.Press));
            menuK.AddItem(new MenuItem("harasskey", "骚扰")).SetValue(new KeyBind(67, KeyBindType.Press));
            menuK.AddItem(new MenuItem("clearkey", "清线")).SetValue(new KeyBind(86, KeyBindType.Press));
            menuK.AddItem(new MenuItem("jumpkey", "跳墙")).SetValue(new KeyBind(88, KeyBindType.Press));
            menuK.AddItem(new MenuItem("changemode", "模式切换")).SetValue(new KeyBind(90, KeyBindType.Press));
            config.AddSubMenu(menuK);

            // Draw settings
            var menuD = new Menu("显示", "dsettings");
            menuD.AddItem(new MenuItem("dsep1", "显示"));
            menuD.AddItem(new MenuItem("drawrr", "R范围")).SetValue(true);
            menuD.AddItem(new MenuItem("drawqq", "Q范围")).SetValue(true);
            ...
            // Combo Settings
            var menuC = new Menu("连招", "csettings");
            ...
            menuC.AddItem(new MenuItem("csep3", "其他"));
            menuC.AddItem(new MenuItem("cancelassist", "连招助手")).SetValue(true);           
            menuC.AddItem(new MenuItem("nostickyq", "使用不同目标Q")).SetValue(false);
            menuC.AddItem(new MenuItem("blockanim", "关闭Q动画")).SetValue(false);
            menuC.AddItem(new MenuItem("qqdelay", "Q突进延迟 ")).SetValue(new Slider(1000, 0, 3000));
            
            config.AddSubMenu(menuC);

            // Extra Settings
            var menuO = new Menu("杂项", "osettings");
            ...
            // Farm/Clear Settings
            var menuJ = new Menu("清线", "jsettings");
            ...
            var rivenD = new Menu("调试", "therivend");
            rivenD.AddItem(new MenuItem("dsep2", "====调试"));
            rivenD.AddItem(new MenuItem("debugdmg", "连招伤害")).SetValue(false);
            rivenD.AddItem(new MenuItem("debugtrue", "真实范围")).SetValue(false);
            rivenD.AddItem(new MenuItem("exportjump", "输出位置")).SetValue(new KeyBind(73, KeyBindType.Press));
            config.AddSubMenu(rivenD);

            var donate = new Menu("作者捐赠信息", "feelo");
            donate.AddItem(new MenuItem("donate", "爱我你就捐赠吧"));
