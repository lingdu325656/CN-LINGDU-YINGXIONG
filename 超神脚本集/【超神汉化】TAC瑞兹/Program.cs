...
namespace LightningRyze
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
			...
			Config = new Menu("【超神汉化】瑞兹", "Twilight Ryze", true);
        ...
        {         
        	target = TargetSelector.GetTarget(Q.Range+25, TargetSelector.DamageType.Magical);
       	...
       	private static void ComboMixed()
        {
        	...
        	if (UseIgnite && (IgniteKillable(target) || GetComboDamage(target) > target.Health))
        		if (IgniteSlot != SpellSlot.Unknown && myHero.Spellbook.CanUseSpell(IgniteSlot) == SpellState.Ready && myHero.Distance(target) <= 600)
					myHero.Spellbook.CastSpell(IgniteSlot, target);    
        ...
        private static void KillSteal()
        {
        	...
        	var UsePacket = Config.Item("UsePacket").GetValue<bool>();
        	if (AutoIgnite && IgniteSlot != SpellSlot.Unknown && myHero.Spellbook.CanUseSpell(IgniteSlot) == SpellState.Ready)
        	{
        		foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(enemy => myHero.Distance(enemy) <= 600 && enemy.IsEnemy && enemy.IsVisible && !enemy.IsDead && IgniteKillable(enemy)))
					myHero.Spellbook.CastSpell(IgniteSlot, enemy);        				
        	}
