...
namespace ChewyMoonsShaco
{
    internal class ChewyMoonShaco
    {
        ...
        {
            (Menu = new Menu("【初见汉化】小丑", "cmShaco", true)).AddToMainMenu();

            // Target Selector
            var tsMenu = new Menu("[初见汉化] - 目标选择器", "cmShacoTS");
            SimpleTs.AddToMenu(tsMenu);
            Menu.AddSubMenu(tsMenu);

            // Orbwalking
            var orbwalkingMenu = new Menu("[初见汉化] - 走砍", "cmShacoOrbwalkin");
            Orbwalker = new LXOrbwalker();
            LXOrbwalker.AddToMenu(orbwalkingMenu);
            Menu.AddSubMenu(orbwalkingMenu);

            // Combo
            var comboMenu = new Menu("[初见汉化] - 连招", "cmShacoCombo");
            ...
            // Harass
            var harassMenu = new Menu("[初见汉化] - 骚扰", "cmShacoHarass");
            harassMenu.AddItem(new MenuItem("useEHarass", "使用 E").SetValue(true));
            Menu.AddSubMenu(harassMenu);

            // Drawing
            var drawingMenu = new Menu("[初见汉化] - 技能范围", "cmShacoDrawing");
            ...
            // Misc
            var miscMenu = new Menu("[初见汉化] - 杂项", "cmShacoMisc");
            miscMenu.AddItem(new MenuItem("usePackets", "使用封包").SetValue(true));
            miscMenu.AddItem(new MenuItem("stuff", "让我知道"));
            miscMenu.AddItem(new MenuItem("stuff2", "你还想添加什么功能"));
            miscMenu.AddItem(new MenuItem("stuff3", "by：初见汉化QQ：5011477"));
