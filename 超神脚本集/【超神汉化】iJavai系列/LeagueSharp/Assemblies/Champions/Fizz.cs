    ...
    internal class Fizz : Champion {
        ...
        private void loadMenu() {
            ...
            menu.SubMenu("combo").AddItem(
                new MenuItem("initR", "起手用R").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
            menu.SubMenu("combo")
            ...
            menu.SubMenu("harass").AddItem(new MenuItem("eTower", "E回最近的塔").SetValue(true));

            menu.AddSubMenu(new Menu("清线", "laneclear"));
            menu.SubMenu("laneclear").AddItem(new MenuItem("useQL", "使用Q").SetValue(false));
            ...
            menu.SubMenu("killsteal").AddItem(new MenuItem("useEKS", "使用E").SetValue(false));

            menu.AddSubMenu(new Menu("逃跑", "flee"));
            menu.SubMenu("flee").AddItem(new MenuItem("useQFlee", "使用Q").SetValue(true));
            menu.SubMenu("flee").AddItem(new MenuItem("useEFlee", "使用E").SetValue(true));

            menu.AddSubMenu(new Menu("偷野", "steal"));
            menu.SubMenu("steal").AddItem(
                new MenuItem("stealKey", "偷小龙").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));

            menu.AddSubMenu(new Menu("杂项", "misc"));
            menu.SubMenu("misc").AddItem(new MenuItem("qWithR", "使用R同时Q").SetValue(false));
            menu.SubMenu("misc").AddItem(new MenuItem("castEGap", "E突进").SetValue(false));
            ...
            menu.SubMenu("draw").AddItem(
                new MenuItem("drawFlee", "显示逃跑点").SetValue(new Circle(true, Color.Cyan)));
            menu.SubMenu("draw").AddItem(new MenuItem("drawQ", "Q范围").SetValue(new Circle(true, Color.Red)));
            menu.SubMenu("draw").AddItem(new MenuItem("drawE", "E范围").SetValue(new Circle(true, Color.Red)));
            menu.SubMenu("draw").AddItem(new MenuItem("drawR", "R范围").SetValue(new Circle(true, Color.Red)));
  ...
  * */
        ...
        private void combo(Obj_AI_Hero t) {
            ...
            if (player.Distance(target) <= Q.Range) {
                ...
                if (isMenuEnabled(menu, "qWithR")) {
                    //IF 使用q with r and not initiate R
        ...
        private void dragonStealerino() {
            ...
            if (E2.IsReady() && player.Distance(stealPosition) < 10 && jumpStage == FizzJump.TRICKSTER &&
                player.Spellbook.CanUseSpell(smite) == SpellState.Cooldown) {
