...
namespace Assemblies.Champions {
    internal class Kalista : Champion {
         ...
         * Suggestions:
            ...
            Q waveclear, when enough rend stacks to kill more than 3 minions 使用Q on killable.(add slider if you want? xD).
            E waveclear lasthitting creeps when they can die from current stacks, its really good seriously, beca使用E resets on minion kill, and cost only 35 mana, you should really add this feature and see how good it is. (with a mana slider). Would add this to priority one xD.
        ...
        private void loadMenu() {
            ...
            //menu.SubMenu("combo").AddItem(new MenuItem("useRC", "使用R in combo").SetValue(true));
            menu.SubMenu("combo").AddItem(new MenuItem("useER", "对正在逃跑的使用E").SetValue(true));
            //menu.SubMenu("combo").AddItem(new MenuItem("useEL", "使用E out of range").SetValue(true));
            ...
            menu.SubMenu("laneclear").AddItem(new MenuItem("autoE", "自动E可杀小兵").SetValue(true));
            menu.SubMenu("laneclear").AddItem(new MenuItem("eNum", "小兵数量").SetValue(new Slider(3, 1, 10)));

            menu.AddSubMenu(new Menu("抢人头", "killsteal"));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useQK", "使用Q").SetValue(true));
            menu.SubMenu("killsteal").AddItem(new MenuItem("useEK", "使用E").SetValue(true));
            ...
            menu.SubMenu("flee").AddItem(new MenuItem("useAAF", "使用平A").SetValue(true));

            menu.AddSubMenu(new Menu("显示", "drawing"));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawQ", "Q范围").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawE", "E范围").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawStacks", "显示E层数").SetValue(false));
            menu.SubMenu("drawing").AddItem(new MenuItem("drawFlee", "显示逃跑位置").SetValue(true));
            ...
            menu.SubMenu("misc").AddItem(new MenuItem("eStacks", "使用E层数").SetValue(new Slider(2, 1, 10)));
        ...
        private void onDraw(EventArgs args) {
            ...
            if (menu.Item("drawStacks").GetValue<bool>()) {
                Obj_AI_Hero target = TargetSelector.GetTarget(Q.Range + E.Range, TargetSelector.DamageType.Physical);
