    ...
    class Vladimir : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("热键", "Key");
            {
                ...
                key.AddItem(new MenuItem("StackE", "叠E(锁定)").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            {
                miscMenu.AddItem(new MenuItem("W_Gap_Closer", "使用W防突").SetValue(true));
                miscMenu.AddItem(new MenuItem("useR_Hit", "R击中(0关闭)").SetValue(new Slider(3, 0, 5)));
                miscMenu.AddItem(new MenuItem("smartKS", "智能抢人头").SetValue(true));
                miscMenu.AddItem(new MenuItem("R_KS", "使用R抢人头").SetValue(true));
                //add to menu
